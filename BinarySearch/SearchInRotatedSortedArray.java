...
public class SearchInRotatedSortedArray {
	...
	// So when doing binary search, we can make a judgment that which part is ordered and 
	// whether the target is in that range, if yes, continue the search in that half, 
	// if not continue in the other half." -- by flyinghx61
	...
	//  1. The drop lies in the right subarray. 
	//  2. The drop lies in the left subarray. 
	//  3. Not rotated. 
	...
	// For Case 1, if nums[left] <= target < nums[mid], then the target lies in the left subarray. 
	// For Case 2, if nums[mid] < target <= nums[right], then the target lies in the right subarray. 
	...
	public int search(int[] nums, int target) {
		...
		while (left <= right) {
			// We can avoid the integer overflow using other version, but.. 
			// for simplicity, this is ok. 
