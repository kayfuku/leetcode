...
public class SplitBST {
	...
	// The output of the recursion stack is the root nodes of left subtree and right subtree.
	// 
	// Return subtree array, where
	// subtrees[0] is always a root node of the subtree that has all the nodes smaller than or equal to V. 
	// subtrees[1] is always a root node of the subtree that has all the nodes bigger than V. 
	// 
	// In other words, 
	...
	public TreeNode[] splitBST(TreeNode node, int V) {
		...
		if (node.val <= V) {
			// In this case, node.left subtree is fine. No need to do something for the current left subtree. 
			// But the node.right subtree might need some modifications. 
			// It does not matter whether node.right is smaller or bigger than V. The next recursion stack  
			// will determine this. 
			subtrees = splitBST(node.right, V);
			// subtrees[0] is always a tree that has all the nodes smaller than or equal to V. 
			node.right = subtrees[0];
			// Output this node as smaller part. 
			subtrees[0] = node;
		} else {
			// node.val > V
			// In this case, node.right subtree is fine. No need to do something for the current right subtree. 
