...
public class DailyTemperatures {
	...
	public int[] dailyTemperatures2(int[] T) {
		int[] ans = new int[T.length];
		// Holds a list of indices representing a strictly increasing list of temperatures. 
		Stack<Integer> stack = new Stack<>();
		for (int i = T.length - 1; i >= 0; i--) {
			// The indices that have values smaller than or equal to T[i] is no longer needed 
			// because earlier index that has higher value is more important than those values. 
			while (!stack.isEmpty() && T[i] >= T[stack.peek()]) {
				// Remove the indices that have values smaller than or equal to T[i]. 
				stack.pop();
			}
			// Assert that stack.peek() returns an index that has the value next larger than T[i]. 
			// stack.peek() - i tells how many days we have to wait until a warmer temperature. 
