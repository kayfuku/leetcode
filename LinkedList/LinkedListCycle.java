...
public class LinkedListCycle {
  ...
  // Two pointers.
  // The fast pointer moves two nodes at a time while the slow pointer moves one
  // node.
  // If the list has a cycle, then fast pointer catches up with the slow pointer
  // at some point, otherwise the fast pointer gets to the last null.
  // O(N) time, O(1) space.

  // https://leetcode.com/problems/linked-list-cycle/solution/
  // Time complexity : O(n). Let us denote n as the total number of nodes in the
  // linked list. To analyze its time complexity, we consider the following two
  // cases separately.
  //
  // List has no cycle:
  ...
  // \text{Number of iterations} = \text{non-cyclic length} = Number of
  ...
  // difference of speed distance between the 2 runners loops for the fast runner
  // to catch up with the slow runner.
  ...
  // \text{almost}Number of iterations=almost "\text{cyclic length K}cyclic length
  // K". Therefore, the worst case time complexity is O(N+K), which is O(n).

  // (Alternative solution?)
  // Let X be the length from head to the start point of cycle, and
  // let Y be the length of the cycle. We know slow moves t, while fast moves 2t.
  ...
  // Space complexity : O(1). We only use two nodes (slow and fast) so the space
  // complexity is O(1).
  public boolean hasCycle2(ListNode head) {
    ...
    while (fast != null && fast.next != null) {
      fast = fast.next.next;
      slow = slow.next;
      // Fast pointer catches up with the slow pointer at some point.
      if (slow == fast) {
        return true;
      }
    }
  ...
  // R3
  public boolean hasCycleR3(ListNode head) {
    if (head == null || head.next == null) {
      return false;
    }
    ListNode slow = head;
