...
public class WordLadder {
	...
	// Review. Accepted. This is the best. 
	// To find a shortest path, build a graph and do BFS. 
	// O(MN) time, where M is the word length and N is the total number of words 
	// in the list because it takes M iterations for each of N words. Also, 
	// BFS in the worst case might go to each of the N words.
	...
	// Review. 
	public int ladderLengthR2(String beginWord, String endWord, List<String> wordList) {
		// 1. Build a graph. 
		Map<String, List<String>> map = new HashMap<>();
		wordList.add(beginWord);
		for (String word : wordList) {
			for (int i = 0; i < word.length(); i++) {
				...
				sb.append("*");
				sb.append(word.substring(i + 1, word.length()));
				String wildcardWord = sb.toString();
				if (!map.containsKey(wildcardWord)) {
					map.put(wildcardWord, new ArrayList<>());
				}
				List<String> neighbors = map.get(wildcardWord);
				neighbors.add(word);
			}
		}
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 0; i < size; i++) {
				...
				for (int j = 0; j < word.length(); j++) {
					...
					String wildcardWord = sb.toString();
					for (String nextWord : map.get(wildcardWord)) {
