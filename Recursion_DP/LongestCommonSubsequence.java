...
public class LongestCommonSubsequence {
	...
	// sequence until i-th character of the first string and the other is the
	// sequence until j-th character of the second string.
	// For each iteration, I check if the i-th character of the first sequence and
	// j-th character of the second sequence are the same.
	//
	...
	// either the length of the LCS we get by dropping the last character of the
	// first sequence or the last character of the second sequence. I take whichever
	// is larger.
	//
	// In other words:
	// If the characters are not the same, then take a larger value of above or left
	// because dp already stores the LCS so far.
	...
	public int longestCommonSubsequence(String s1, String s2) {
		int m = s1.length();
		int n = s2.length();
		// To keep track of the longest common subseq length.
		...
		// Return the most bottom right value.
		return dp[m][n];
	}
